"""
Full Demo Evaluation — restarts fresh. Batches of 4 concurrent queries.
"""
...
def check_kb_has_answer(query, kb):
    ...
    topics = [
        ...
        ["fee", "fees", "cost", "tuition", "scholarship"],
        ["hostel", "mess", "room", "capacity"],
        ["placement", "job", "recruit", "company", "package", "career"],
        ["department", "branch", "course", "program", "b.tech", "mca", "mba", "cse", "aiml"],
        ["faculty", "teacher", "professor", "hod", "principal"],
        ["campus", "acre", "wifi", "library", "laboratory", "lab", "facility"],
        ["scholarship", "tfw", "kanyashree"],
        ["document", "marksheet", "certificate", "admit card", "allotment"],
        ["nba", "naac", "accredited", "established"],
        ["fail", "backlog", "laptop", "dress code", "attendance", "refund", "caution"],
        ["contact", "portal", "website", "phone", "email"],
        ["online", "virtual", "remote"],
        ["international", "foreign", "visa", "dasa"],
        ["refund", "caution money", "security deposit", "cancellation"],
    ]
...
async def main():
    ...
    lines.append(
        f"  Estimated accuracy (KB-answerable): {correct}/{answerable} = {correct / max(1, answerable) * 100:.1f}%"
    )
    ...
    for cat in sorted(cats.keys()):
        items = cats[cat]
        h = sum(1 for r in items if r["hallucination"] == "YES")
        c = sum(1 for r in items if r["context_len"] > 0)
        l = sum(r["latency_ms"] for r in items) / len(items)
        lines.append(f"    {cat:15s} n={len(items):2d} ctx={c:2d} hall={h:2d} lat={l:.0f}ms")
    lines.append(f"")
    ...
    for lang in sorted(langs.keys()):
        ...
        c = sum(1 for r in items if r["context_len"] > 0)
        l = sum(r["latency_ms"] for r in items) / len(items)
        lines.append(f"    {lang:10s} n={len(items):2d} ctx={c:2d} hall={h:2d} lat={l:.0f}ms")
    lines.append(f"")
    ...
    report = "\n".join(lines)
    report_path = (
        Path("backend/benchmarks/eval_reports") / f"demo_eval_{time.strftime('%Y%m%d_%H%M%S')}.txt"
    )
